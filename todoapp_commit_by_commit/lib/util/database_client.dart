...
//insertion
  Future<int> saveItem(todo_item item) async {
...
//
  Future<todo_item> getItem(int id) async {
    var dbClient = await db;
    var result =
        await dbClient.rawQuery("SELECT * FROM $tableName WHERE id = $id");
    if (result.length == 0) return null;
    return new todo_item.fromMap(result.first);
  }
...
//  }
  ...
  Future<int> updateItem(todo_item item) async {
    var dbClient = await db;
